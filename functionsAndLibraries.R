...
prepareEnrichments <- function(x, threshold = 0.05, foldChange=1.5, type="rankedList", collectionId="symbol", regulons=T, 
  ...
  if(collectionId == "entrez"){
    ...
    h <- read.gmt(gmtfile = "/mnt/data/mcangiano/7060_Glasgow/RNAseq/Data_analyis/networkAnalysis/startData/h.all.v6.2.symbols.gmt")
    c3 <- read.gmt(gmtfile = "/mnt/data/mcangiano/7060_Glasgow/RNAseq/Data_analyis/networkAnalysis/startData/c3.all.v6.2.symbols.gmt") 
  }
  if(type == "rankedList"){
...
topologyAnalysis2 <- function(h, netReg, degOutput=NA, outFile="prova.rda", treatments,
  ...
  if(singleSample){
    ...
    nbea.res <- foreach(.errorhandling = "pass", i= 1:length(treatments), .packages = c("EnrichmentBrowser", "SummarizedExperiment"), .options.snow = opts)%dopar%{
      ...
      if(convert){
        load("F://mcangiano/Glasgow/RNAseq_orthografts/startData/convertedGenesFromRawCountsFromMygene.rda")
  ...
  return(lapply(nbea.res, FUN = function(x){y <- x$res.tbl}))
}
