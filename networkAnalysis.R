...
setwd("D://mcangiano/Glasgow/RNAseq_orthografts/")
...
load("D:/mcangiano/Glasgow/RNAseq_orthografts/RTN/standardisedCounts_pearson/standardisedNormCountsConditional.rda")
...
regressionsRank <- read.csv("D://mcangiano/survivalAnalysis/table_paper/coxInternalDatasets.csv", header = T, stringsAsFactors = F)
...
names(myRank2) <- sapply(names(myRank2), function(x){unlist(strsplit(x, "_"))[2]})
verCol <- myRank2[V(myGraph)$name]
verCol[is.na(verCol)] <- "black"
load("D://mcangiano/Glasgow/RNAseq_orthografts/startData/tfListFrancesca.rda")
newTFs <- newTFs[!is.na(newTFs)] 
newTFs <- newTFs[!duplicated(newTFs)]
...
plot(myGraph, vertex.label=ifelse(V(myGraph)$name %in% names(newTFs), V(myGraph)$name, NA), 
     ...
     edge.width=1, vertex.label.cex = 1, vertex.label.font=2, vertex.label.color="black",
